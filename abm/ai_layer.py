...
def apply_ai_modifiers(transition_matrix: list[list[float]], ai_mode: AIMode) -> list[list[float]]:
    """Applies AI speed and rework modifiers to a given transition matrix.
    ...
    T[FBSState.REQUIREMENTS][FBSState.FUNCTIONS] *= AI_SPEED
    T[FBSState.FUNCTIONS][FBSState.EXPECTED_BEHAVIOR] *= AI_SPEED
    T[FBSState.EXPECTED_BEHAVIOR][FBSState.STRUCTURE] *= AI_SPEED
    T[FBSState.STRUCTURE][FBSState.DOCUMENTATION] *= AI_SPEED

    if ai_mode == AIMode.AI_RAW:
        # High unmitigated rework
        T[FBSState.STRUCTURE][FBSState.EXPECTED_BEHAVIOR] *= AI_REWORK
        T[FBSState.FUNCTIONS][FBSState.REQUIREMENTS] *= AI_REWORK
        T[FBSState.STRUCTURE][FBSState.STRUCTURE] *= AI_REWORK
    elif ai_mode == AIMode.AI_REACT:
        # Apply ReAct logic for rework mitigation
        react_gate = ReActGate()
        T = react_gate.apply_react(T)
    ...
    for i in range(len(T)):
        ...
        if non_diagonal_sum >= 1.0:
            ...
            T[i][diagonal_index] = min(1.0, max(0.0, T[i][diagonal_index]))
